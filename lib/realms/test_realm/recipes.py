...
def data_fetch_plan(
    ...
    """
    Planning-time CouchDB fetch recipe.

    The handler intercepts this recipe name (Mode 1) and calls
    ``data_fetch_plan_steps()`` with the result of an async
    ``ctx.data.connection("yggdrasil_db").get("data_access_test:reference_doc")``
    performed during ``generate_plan_drafts()``. The ``ref_doc`` baked into
    ``echo_fetched.params["ref_doc"]`` is therefore resolved at plan-generation
    time, not during execution.

    This registry entry makes the recipe discoverable. When called directly
    (e.g. in tests), it generates the same step shape with a placeholder
    ``ref_dict`` indicating the doc has not yet been fetched.

    Steps (generated by the handler with actual planning data):
        1. echo_fetched: Emits the plan-time ref_doc via step_emit_metadata
        2. echo_confirm: Echoes the outcome message

    Args:
        overrides: Optional param overrides by step_id.

    Returns:
        List of StepSpec (placeholder ref_dict; handler replaces at plan time).
    """
    overrides = overrides or {}
    steps = data_fetch_plan_steps(
        ref_dict={"doc_id": "data_access_test:reference_doc", "missing": True}
    )
    return _apply_overrides(steps, overrides)
